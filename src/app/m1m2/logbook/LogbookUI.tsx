...
/* ============================
   Helpers de API (você já colou até o step 4.2)
============================ */
...
const LOGO_SRC = '/ame-one-logo.png'; // coloque em /public
...
export default function LogbookUI({ logbookId }: { logbookId: string }) {
  ...
  const [licenceType, setLicenceType] = useState<'M1/M2' | 'E' | 'S' | 'B'>(
    'M1/M2',
  );
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (saved) {
        ...
        if (parsed?.licenceType) setLicenceType(parsed.licenceType);

        // ⚠️ Mantém compatibilidade com versões antigas (pode vir "licence"/"date" etc)
        ...
        if (Array.isArray(parsed?.dateCompleted)) setDateCompleted(parsed.dateCompleted);
        if (Array.isArray(parsed?.aircraftOrComponent)) setAircraftOrComponent(parsed.aircraftOrComponent);
        if (Array.isArray(parsed?.regOrSerial)) setRegOrSerial(parsed.regOrSerial);
        if (Array.isArray(parsed?.initialsField)) setInitialsField(parsed.initialsField);
        if (Array.isArray(parsed?.selectedSignatoryIndex)) setSelectedSignatoryIndex(parsed.selectedSignatoryIndex);
        if (Array.isArray(parsed?.signedBySignatoryIndex)) setSignedBySignatoryIndex(parsed.signedBySignatoryIndex);
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // se falhar, segue com o state local
      }
    })();
  }, [logbookId]);
  ...
  const progressPercent =
    totalApplicable === 0 ? 0 : Math.round((signedCount / totalApplicable) * 100);
  ...
  const markGroupNA = (groupIndex: number) => {
    ...
    setTaskStatus((prev) => prev.map((s, i) => (i >= start && i <= end ? 'na' : s)));
  };

  const undoGroupNA = (groupIndex: number) => {
    const { start, end } = GROUP_RANGES[groupIndex];
    setTaskStatus((prev) =>
      prev.map((s, i) => (i >= start && i <= end && s === 'na' ? 'pending' : s)),
    );
  };
  ...
  const signatoryOptions = useMemo(() => {
    return signatories
      .map((s, idx) => ({ idx, ...s }))
      .filter((s) => (s.name?.trim() ?? '') !== '' || (s.initials?.trim() ?? '') !== '')
      .map((s) => ({
        value: s.idx,
        label: `${s.idx + 1} — ${
          s.initials?.trim() ? s.initials.trim() : s.name?.trim() || 'Signatory'
        }`,
      }));
  }, [signatories]);
  ...
  const handleSelectSignatory = (rowIndex: number, signIdx: number) => {
    ...
    const autoInitials = signIdx >= 0 ? (signatories[signIdx]?.initials ?? '').trim() : '';
  ...
  const addCertifiedCopies = (signatoryIndex: number, files: FileList | null) => {
    ...
    const imageFiles = Array.from(files).filter((f) => /^image\/(png|jpe?g|webp)$/i.test(f.type));
  ...
  const btnDisabled = 'border-gray-200 bg-gray-100 text-gray-400 cursor-not-allowed';
  ...
  // ✅ Header do conteúdo (não aparece em capa/contracapa)
  ...
  return (
    <div className="p-4 md:p-8 flex justify-center">
      <div className="w-full max-w-5xl bg-white text-[11px] leading-snug border border-black">
        ...
        <div className="screen-only sticky top-0 z-10 bg-white border-b border-gray-200">
          <div className="flex items-center justify-between px-3 py-2">
            ...
            <div className="flex items-center gap-2">
              <select
                ...
                value={licenceType}
                onChange={(e) =>
                  setLicenceType(e.target.value as 'M1/M2' | 'E' | 'S' | 'B')
                }
          ...
          <div className="px-3 pb-2">
            ...
            <div className="mt-1 h-2 w-full rounded bg-gray-200 overflow-hidden">
              <div className="h-full bg-gray-700" style={{ width: `${progressPercent}%` }} />
            </div>
          </div>
        </div>

        {/* ✅ COVER (PRINT ONLY) — sem Applicant/File */}
        <div className="print-only break-after-page">
          <div className="border border-black min-h-[1050px] p-10 relative overflow-hidden">
            ...
            <div className="relative">
              ...
              <div className="text-[42px] font-black mt-3 text-center">LOGBOOK</div>
        ...
        {/* ✅ From here on: content pages (print) should show applicant/file */}
        <PrintContentHeader />
        ...
        <div className="px-2 py-2 border-b border-black">
          ...
          <p>
            The Maintenance Task List below is based on paragraph 566.03(4)(e) and Appendix B of Chapter
            566 of the Airworthiness. Applicants utilizing this document, including persons authorized
            to certify the completion of tasks, should be familiar with these requirements to properly
            complete this document as a means for an applicant to establish proof of skill.
          </p>
          <p className="mt-1">
            Applicants cannot sign for their own maintenance tasks regardless of whether they hold a
            rating, foreign licence, or authority within an organization to sign a maintenance release
            or equivalent release.
          </p>
          <p className="mt-1">
            Only tasks completed which have been subject to a maintenance release or equivalent are
            acceptable. Tasks completed are to be initialled by the holder of an AME licence or
            equivalent person attesting to its completion by the applicant, and that the person has
            observed the work to the extent necessary and confirm that the applicant has completed the
            tasks in accordance with the standards of airworthiness.
          </p>
          <p className="mt-1">
            The applicant is responsible to ensure that the tasks performed are applicable to the
            rating sought and to the aircraft, systems or components for which the experience is being
            claimed.
          </p>
          <p className="mt-1">
            Where a maintenance task has been initialled by a foreign AME licence holder or by an
            equivalent person holding an authority such as a Shop Certification Authority (SCA), the
            applicant is to provide certified true copies of the person’s foreign licence or authority
            along with this document for review.
          </p>
          <p className="mt-1">
            Each signatory is required to complete the information below (limit of 15 signatories):
          </p>
        </div>
        ...
        <table className="w-full border-b border-black border-collapse">
          <thead>
            <tr>
              <th className="border-t border-b border-black px-1 py-1 w-[5%]">#</th>
          ...
          <tbody>
            {signatoryRows.map((idx) => {
              ...
              return (
                <tr key={idx}>
                  <td className="border-t border-black px-1 py-1 text-center">{idx + 1}</td>
                  ...
                  <td className="border-t border-l border-black px-1 py-1">
                    ...
                    <div className="screen-only mt-2">
                      <label className={uploadBtn} title="Upload supporting documentation (images)">
                  ...
                  <td className="border-t border-l border-black px-1 py-1">
                    <input
                      ...
                      onChange={(e) => updateSignatory(idx, { licenceNumber: e.target.value })}
                      onBlur={async () => {
                        try {
                          await saveSignatoryRow(idx);
                        } catch {}
                  ...
                  <td className="border-t border-l border-black px-1 py-1">
                    <input
                      type="text"
                      value={s.initials}
                      onChange={(e) => updateSignatory(idx, { initials: e.target.value })}
                      onBlur={async () => {
                        try {
                          await saveSignatoryRow(idx);
                        } catch {}
                  ...
                  <td className="border-t border-l border-black px-1 py-1">
                    ...
                    {/* ✅ NOTE: No “Draw signature” here.
                        Signature is provided ONLY by the signatory via mobile verification link. */}
                  ...
                  <td className="border-t border-l border-black px-1 py-1">
                    <input
                      type="date"
                      value={s.dateSigned}
                      onChange={(e) => updateSignatory(idx, { dateSigned: e.target.value })}
                      onBlur={async () => {
                        try {
                          await saveSignatoryRow(idx);
                        } catch {}
              ...
              <div key={`support-${idx}`} className="break-after-page">
                ...
                {items.map((c, i) => (
                  <div key={`${c.file.name}-${i}`} className="break-after-page">
                    ...
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={c.url} alt={c.file.name} className="w-full border border-black" />
                  </div>
                ))}
              </div>
            );
        ...
        <div className="px-2 py-2 border-b border-black">
          <div className="font-semibold mb-1">Instructions to complete the maintenance task list:</div>
          <ul className="list-disc pl-5">
            <li>
              <strong>Date completed</strong> – year, month, day, e.g., YYYY-MM-DD.
            </li>
            <li>
              <strong>Aircraft type</strong> – model based on aircraft Type Certificate Data Sheet (TCDS).
            </li>
            <li>
            ...
            <li>
              <strong>Component serial number</strong> – serial number from the component data plate.
            </li>
            <li>
              <strong>Initials of AME or equivalent person</strong> – initials of a person listed in signatory table.
            </li>
          </ul>
        </div>
        ...
        <table className="w-full border-t border-black border-collapse">
          <thead>
            <tr>
              ...
              <th className="border-b border-l border-black px-2 py-1 text-left">Maintenance Tasks</th>
              <th className="border-b border-l border-black px-2 py-1 w-[14%]">Date Completed</th>
              <th className="border-b border-l border-black px-2 py-1 w-[18%]">Aircraft Type or Component</th>
              ...
              return (
                <React.Fragment key={`${group.ata}-${groupIndex}`}>
                  <tr>
                    ...
                    >
                      <div className={cx('relative pr-10', isExpanded ? 'leading-snug' : 'leading-tight')}>
                        <div>ATA {group.ata}</div>
                        <div className={cx('font-normal', isExpanded ? 'text-[11px]' : 'text-[10px]')}>
                          {group.title}
                        </div>
                    ...
                    <td className={cx('border-t border-l border-black px-2', isExpanded ? 'py-2' : 'py-1')}>
                      {isExpanded && (
                        <div className="screen-only flex justify-end gap-2">
                          <button type="button" onClick={() => undoGroupNA(groupIndex)} className={cx(btnBase, btnInactive)}>
                            Reset N/A
                          </button>
                          <button type="button" onClick={() => markGroupNA(groupIndex)} className={cx(btnBase, btnInactive)}>
                            N/A Chapter
                          </button>
                        </div>
                      )}
                  ...
                  {isExpanded &&
                    group.tasks.map((task, index) => {
                      ...
                      const signedSigSvg = signedIdx >= 0 ? (signatories[signedIdx]?.signatureSvg ?? '') : '';
                        ...
                        <tr key={`${group.ata}-${index}`}>
                          ...
                          <td className="border-t border-l border-black px-2 py-1">
                            <div className="flex items-start justify-between gap-2">
                              <span>• {task}</span>
                              <span className="screen-only text-[10px] text-gray-500">{getStatusLabel(status)}</span>
                            </div>
                          </td>
                          ...
                          <td className="border-t border-l border-black px-1 py-1">
                            ...
                            {signActive && signedSigSvg && (
                              <div
                                ...
                                style={{ padding: '2px' }}
                              >
                                <div className="w-full h-full" dangerouslySetInnerHTML={{ __html: signedSigSvg }} />
                              </div>
                            )}
                            ...
                            <div className="screen-only mt-1 flex flex-col gap-1">
                              <div className="flex gap-1">
                                <select
                                  ...
                                  onChange={(e) => handleSelectSignatory(rowIndex, Number(e.target.value))}
                                >
                                ...
                                <button
                                  ...
                                  onClick={() => {
                                    ...
                                    // ✅ Só deixa "Sign" se o signatory estiver VERIFIED
                                    if ((signatories[sigIdx]?.status || 'DRAFT') !== 'VERIFIED') {
                                      showSignWarning(rowIndex);
                                      return;
                                    }

                                    const autoInitials = (signatories[sigIdx]?.initials ?? '').trim();
                                    if (autoInitials && !initialsField[rowIndex]) {
                                      setInitialsField((prev) => {
                                        const next = [...prev];
                                        next[rowIndex] = autoInitials;
                              ...
                              <div className="text-[10px] text-gray-500">Status: {getStatusLabel(status)}</div>
                            </div>
                          </td>
                        </tr>
                      );
        ...
        {/* ✅ BACK COVER (PRINT ONLY) — sem Applicant/File */}
